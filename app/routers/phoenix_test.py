...
class EmotionClassifyRequest(BaseModel):
    ...
    prosody_context: Optional[str] = Field(
        None, description="Optional prosody analysis (e.g., 'pitch: high, pace: fast')"
    )
...
async def classify_emotion(
    request: EmotionClassifyRequest, _token: str = Depends(verify_api_key)
    ...
    try:
        async with PhoenixClient(config) as client:
            result: PhoenixResult = await client.classify(
                text=request.text, prosody_context=request.prosody_context
            )
