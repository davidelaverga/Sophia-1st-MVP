...
class SharedServiceManager:
    ...
    def get_hybrid_voxtral_service(cls) -> Optional[HybridVoxtralService]:
        ...
        if cls._hybrid_voxtral_service is None:
            try:
                ...
                logger.info(
                    "SharedServiceManager: HybridVoxtralService initialized successfully"
                )
            except Exception as e:
                logger.warning(
                    f"SharedServiceManager: Failed to initialize HybridVoxtralService: {e}"
                )
    ...
    def get_session_turn_manager(cls) -> SessionTurnManager:
        ...
        if cls._session_turn_manager is None:
            ...
            logger.info(
                "SharedServiceManager: SessionTurnManager initialized successfully"
            )
