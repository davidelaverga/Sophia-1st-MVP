 ...
 * 
 * Proxies cancellation request to backend's /text-chat/{session_id}/cancel
...
export async function POST(
  ...
  try {
    const response = await fetch(
      `${BACKEND_URL}/text-chat/${sessionId}/cancel`,
      {
        method: "POST",
        headers: {
    ...
    console.error("[cancel] Backend request failed:", error)
