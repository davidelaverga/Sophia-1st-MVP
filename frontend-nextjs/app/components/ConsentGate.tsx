...
export function ConsentGate({ onReady }: { onReady: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async () => {
      ...
      try {
        ...
        if (aborted) return
        setState("error")
        setError((err as Error).message || "Unable to load consent status.")
      }
    }
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      setState("needsConsent")
      setError((err as Error).message || "We could not save your consent.")
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-sophia-bg px-3">
      ...
      >
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <button
            type="button"
            className="inline-flex flex-1 items-center justify-center rounded-2xl border border-sophia-text/20 px-4 py-3 text-sm font-medium text-sophia-text transition hover:border-sophia-purple/40"
        ...
        {showFallbackContinue && (
          <button
            ...
            onClick={onReady}
          >
            Continue anyway (we will ask again soon)
          </button>
        )}
      </div>
    </div>
